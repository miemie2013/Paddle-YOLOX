...
for nid, num in enumerate(nums):
    ...
    for kk in range(num):
        ...
        if (nid + 2) in dcn_v2_stages:   # DCNv2
            ...
            conv_name = 'backbone.stage%d_%d.conv2.conv_offset' % (2+nid, kk)
...
for lvl in range(0, num_convs):
    ...
    if lvl in dcn_v2_stages:  # DCNv2
        ...
        copy_conv('head.krn_convs.%d.conv_offset' % (lvl, ), offset_w, offset_b)
    ...
    if lvl in dcn_v2_stages:  # DCNv2
        ...
        copy_conv('head.cls_convs.%d.conv_offset' % (lvl, ), offset_w, offset_b)
