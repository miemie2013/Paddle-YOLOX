...
class Decode_YOLOX(object):
    def __init__(self, model, all_classes, place, cfg, for_test=True):
        ...
        self.resizeImage = ResizeImage(target_size=target_size, resize_box=False, interp=cfg.yOLOXResizeImage['interp'],
                                       max_size=max_size, use_cv2=cfg.yOLOXResizeImage['use_cv2'])
        self.squareImage = SquareImage(**cfg.squareImage)
    ...
    def detect_image(self, image, pimage, im_info, draw_image, draw_thresh=0.0):
        ...
        if len(pred[0]) == 0 or np.sum(pred[0]) < 0:
            boxes = np.array([])
            classes = np.array([])
            scores = np.array([])
        else:
    ...
    def multi_thread_post(self, i, pred, result_image, result_boxes, result_scores, result_classes, batch_img, draw_image, draw_thresh):
        if len(pred[i]) == 0 or np.sum(pred[i]) < 0:
            boxes = np.array([])
            classes = np.array([])
            scores = np.array([])
        else:
    ...
    def process_image(self, img):
        ...
        sample = self.squareImage(sample, context)
